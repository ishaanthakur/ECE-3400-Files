...
static const uint8_t START_X = 0; // starting position x coordinate
static const uint8_t START_Y = 0; // starting position y coordinate
...
static const uint8_t TEMP_WALLS = 0; // same as WALLS, except these disappear after each mapping cycle. they represent robots that must be avoided
...
static node *robot_node = &(maze[START_X][START_Y]); // current robot node, initialized to starting position
static heading robot_hdg; // current robot heading
static RF24 radio(RF_CE_PIN, RF_CS_PIN); // radio object. uses SPI bus + pins 9 and 10
...
node* greedy() {
  ...
  while(fifo_front) { // while there are still frontier nodes to analyze...
    ...
    heading bfs_hdg; // heading robot would have upon reaching that node
    if(bfs_node == robot_node) { // if it's the current robot node...
      bfs_hdg = robot_hdg; // the heading is the current robot heading
    }
    else {
...
/* Extract the knowledge bytes from the nodes and place them in a buffer */
  
  for(uint8_t x = 0; x < n; x++) {
    for(uint8_t y = 0; y < n; y++) {
      buff[y * n + x] = maze[x][y].data;
    }
  }
}
...
void setup() {
  ...
  waitForStart(); // wait for start signal or button press

  adcAnalogReadInit(); // initialize ADC for analogRead()
  servoInit(); // initialize servos
  rfInit(&radio); // initialize radio
...
void loop() {
  ...
  char maze_data_buff[n * n]; // buffer for maze data to be trasnmitted
  prepareData(maze_data_buff); // extract the knowledge from the nodes into the buffer

  cleanNodes(); // remove temporary node information
  Serial.println();
  
  if(!next_step) { // if robot visited all nodes...
    if(getPos(robot_node).x != START_X || getPos(robot_node).y != START_Y) { // if it's not at the starting position...
      greenLEDOn(); // turn on green LED
      maze[START_X][START_Y].data &= ~(0b1 << VISITED); // mark starting node unvisited so that algorithm finds the way back
    }
    else {
      while(1); // otherwise wait forever
    }
  }
  else {
    robot_hdg = setHDG(calcHDG(robot_node, next_step)); // otherwise turn towards the next node
    uint8_t status = advanceOne(&radio, maze_data_buff); // try to reach the next node while transmitting the maze to the base station
    if(status == 0) { // if next node reached sucessfully...
      robot_node = next_step; // update robot_node
    }
    else {
      backUp(); // otherwise (if another robot blocks the path), reverse to the original node
